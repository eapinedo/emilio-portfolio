...
import { In as Texture, Jn as Vector3, Ut as PerspectiveCamera, Vt as OrthographicCamera, d as BufferGeometry, er as WebGLRenderer, hn as Raycaster, nr as three_module_exports, ot as Layers, p as Camera, qn as Vector2, v as Clock, xn as Scene, xt as Material, y as Color } from "./three.module-C6DorMKW.js";
...
//#endregion
//#region node_modules/@react-three/fiber/dist/events-321b05fb.esm.js
...
function applyProps$1(instance, data) {
	...
	for (let i = 0; i < changes.length; i++) {
		...
		if (isEvent && localState) {
			...
			if (currentInstance[key] instanceof Texture && currentInstance[key].format === 1023 && currentInstance[key].type === 1009 && rootState) {
...
function createRoot(canvas) {
	...
	return {
		configure(props = {}) {
			...
			if (!state.scene) {
				let scene;
				if (sceneOptions instanceof Scene) scene = sceneOptions;
				else {
					scene = new Scene();
...
var import_debounce = /* @__PURE__ */ __toESM((/* @__PURE__ */ __commonJSMin(((exports, module) => {
	/**
...
function useMeasure({ debounce, scroll, polyfill, offsetSize } = {
	debounce: 0,
	scroll: false,
	offsetSize: false
}) {
	...
	function removeListeners() {
		...
		if (state.current.orientationHandler) {
			...
			else if ("onorientationchange" in window) window.removeEventListener("orientationchange", state.current.orientationHandler);
		}
	}
	function addListeners() {
		...
		(_state$current$resize = state.current.resizeObserver) == null || _state$current$resize.observe(state.current.element);
		...
		if ("orientation" in screen && "addEventListener" in screen.orientation) screen.orientation.addEventListener("change", state.current.orientationHandler);
		else if ("onorientationchange" in window) window.addEventListener("orientationchange", state.current.orientationHandler);
	}
	const ref = (node) => {
...
//# sourceMappingURL=react-three-fiber.esm-BfrnwkHN.js.map